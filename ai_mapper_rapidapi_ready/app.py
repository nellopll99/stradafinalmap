...
rapidapi_key = st.text_input("🌐 RapidAPI Bing Key", type="password")
...
if st.button("Cerca aziende"):
    if not openai_key or not prompt or not rapidapi_key:
        st.error("❌ Inserisci chiave OpenAI, RapidAPI e descrizione.")
    else:
        with st.spinner("🔍 Ricerca in corso..."):
            results = search_web(prompt, max_results=max_results, rapidapi_key=rapidapi_key)
